...
public class SessionService(ISessionRepository sessionRepository, IOllamaModelDefinitionProvider modelDefinitionProvider) : ISessionService
{
    ...
    public async Task UpdateSessionStateAsync(Guid sessionId, SessionState state, CancellationToken cancellationToken)
    {
        var session = await sessionRepository.GetByIdAsync(sessionId, cancellationToken);
        if (session != null) session.State = state;
    }

    public Task CreateOrUpdateSession(Guid sessionId, OllamaModelDefinition configuration, CancellationToken token)
    {
